...
@Slf4j
public class EmbeddingSettingsSpecification extends BaseSpecification<EmbeddingSettingsEntity, EmbeddingSettingsRequest> {
    
    public static Specification<EmbeddingSettingsEntity> search(EmbeddingSettingsRequest request, AuthService authService) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            // 使用带层级过滤的基础条件
            predicates.addAll(getBasicPredicatesWithLevel(root, criteriaBuilder, request, authService, EmbeddingSettingsPermissions.MODULE_NAME));
            // name
            ...
            if (request.getDefaultSettings() != null) {
                predicates.add(criteriaBuilder.equal(root.get("defaultSettings"), request.getDefaultSettings()));
            }
