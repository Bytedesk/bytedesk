...
public class SpringAIBaiduService extends BaseSpringAIService {
    ...
    private final Optional<OpenAiChatModel> baiduChatModel;

    public SpringAIBaiduService(
            Optional<OpenAiChatModel> baiduChatModel,
        ...
        super(springAIVectorService, messageSendService, uidUtils, robotRestService, threadRestService,
                messagePersistCache);
        this.baiduChatModel = baiduChatModel;

    }

    @Override
    protected void processPrompt(Prompt prompt, MessageProtobuf messageProtobuf) {
        baiduChatModel.ifPresent(model -> model.stream(prompt).subscribe(
                response -> {
                    if (response != null) {
    ...
    protected String generateFaqPairs(String prompt) {
        return baiduChatModel.map(model -> model.call(prompt)).orElse("");
    }

    @Override
    protected String processPromptSync(String message) {
        try {
            return baiduChatModel.map(model -> model.call(message))
                    .orElse("Baidu service is not available");
        } catch (Exception e) {
    ...
    protected void processPromptSSE(Prompt prompt, MessageProtobuf messageProtobuf, SseEmitter emitter) {
        baiduChatModel.ifPresentOrElse(
                model -> {
