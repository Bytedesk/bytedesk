...
public class SpringAIRagflowService extends BaseSpringAIService {
    ...
    protected void processPromptWebsocket(Prompt prompt, RobotProtobuf robot, MessageProtobuf messageProtobufQuery,
        ...
        try {
            log.info("Processing RAGFlow prompt via WebSocket for robot: {}", robot.getUid());
            
            String content = fullPromptContent;
            String chatId = getChatId(robot, messageProtobufQuery);
            
            // 使用RAGFlow Chat API发送消息
            String response = ragflowChatService.sendMessage(chatId, content, false);
            
            // 解析响应并发送
            String answer = ragflowChatService.extractMessageContent(response);
            ...
            log.error("Error processing RAGFlow prompt via WebSocket", e);
    ...
    protected String processPromptSync(String message, RobotProtobuf robot, String fullPromptContent) {
        try {
            log.info("Processing RAGFlow prompt synchronously for robot: {}", robot.getUid());
            ...
            log.error("Error processing RAGFlow prompt synchronously", e);
    ...
    protected void processPromptSse(Prompt prompt, RobotProtobuf robot, MessageProtobuf messageProtobufQuery,
        ...
        try {
            log.info("Processing RAGFlow prompt via SSE for robot: {}", robot.getUid());
            
            String content = fullPromptContent;
            String chatId = getChatId(robot, messageProtobufQuery);
            
            // 对于SSE，我们使用非流式模式然后发送完整响应
            // RAGFlow支持streaming，但这里简化处理
            String response = ragflowChatService.sendMessage(chatId, content, false);
            ...
            log.error("Error processing RAGFlow prompt via SSE", e);
            try {
    ...
    private void recordTokenUsage(RobotProtobuf robot, String response, int inputLength, int outputLength) {
        try {
            // 尝试从RAGFlow响应中提取实际的token使用情况
            java.util.Map<String, Object> usage = ragflowChatService.extractUsage(response);
            ...
            LlmTokenUsageEvent event = new LlmTokenUsageEvent(
                ...
                LlmConsts.RAGFLOW, // aiProvider
                "ragflow-default", // aiModelType
            ...
            log.warn("Failed to record token usage for RAGFlow", e);
        }
    }
}
