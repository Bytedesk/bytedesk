...
public class VectorDBFactory {
    ...
    private static final Logger logger = LoggerFactory.getLogger(VectorDBFactory.class);
    ...
    public void init() {
        ...
        if (chromaEnabled) {
            try {
                ...
                logger.info("Using Chroma as the primary vector database");
            } catch (Exception e) {
                logger.warn("Failed to initialize Chroma vector database, falling back to Redis", e);
            }
        } else if (elasticsearchEnabled) {
            try {
                this.primaryVectorDBService = context.getBean(ElasticsearchVectorDBService.class);
                logger.info("Using Elasticsearch as the primary vector database");
            } catch (Exception e) {
                logger.warn("Failed to initialize Elasticsearch vector database, falling back to Redis", e);
            }
        } else if (weaviateEnabled) {
            try {
                this.primaryVectorDBService = context.getBean(WeaviateVectorDBService.class);
                logger.info("Using Weaviate as the primary vector database");
            } catch (Exception e) {
                logger.warn("Failed to initialize Weaviate vector database, falling back to Redis", e);
            }
        } else if (milvusEnabled) {
            try {
                this.primaryVectorDBService = context.getBean(MilvusVectorDBService.class);
                logger.info("Using Milvus as the primary vector database");
            } catch (Exception e) {
                logger.warn("Failed to initialize Milvus vector database, falling back to Redis", e);
            }
        } else {
            logger.info("Using Redis as the primary vector database");
        }
    }
