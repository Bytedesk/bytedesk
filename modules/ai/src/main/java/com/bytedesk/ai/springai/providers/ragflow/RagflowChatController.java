...
public class RagflowChatController {
    ...
    public ResponseEntity<?> createChatCompletion(@PathVariable String chatId,
                                                @RequestBody RagflowChatRequest request) {
        try {
            log.info("Chat completion request for chatId: {}", chatId);
            
            String response = ragflowChatService.createChatCompletion(
                chatId,
                request.getModel(),
                request.getMessages(),
                request.getStream()
            ...
            // 检查是否有错误
            if (ragflowChatService.isError(response)) {
                String errorMessage = ragflowChatService.getErrorMessage(response);
                int errorCode = ragflowChatService.getErrorCode(response);
    ...
    public ResponseEntity<?> createAgentCompletion(@PathVariable String agentId,
        ...
        try {
            ...
            String response = ragflowChatService.createAgentCompletion(
                agentId,
                request.getModel(),
                request.getMessages(),
                request.getStream()
    ...
    public ResponseEntity<?> sendChatMessage(@PathVariable String chatId,
                                           @RequestBody RagflowMessageRequest request) {
        try {
            log.info("Send message request for chatId: {}", chatId);
            
            String response = ragflowChatService.sendMessage(
                chatId,
                request.getContent(),
                request.getStream()
            );
            ...
            // 解析响应内容
            String messageContent = ragflowChatService.extractMessageContent(response);
            String finishReason = ragflowChatService.extractFinishReason(response);
            Map<String, Object> usage = ragflowChatService.extractUsage(response);
    ...
    public ResponseEntity<?> sendAgentMessage(@PathVariable String agentId,
        ...
        try {
            ...
            String response = ragflowChatService.sendAgentMessage(
                agentId,
                request.getContent(),
                request.getStream()
            );
    ...
    public ResponseEntity<?> quickChat(@PathVariable String chatId,
        ...
        try {
            String response = ragflowChatService.sendMessage(chatId, content, stream);
    ...
    public ResponseEntity<?> quickAgentChat(@PathVariable String agentId,
                                          ...
                                          @RequestParam(required = false, defaultValue = "false") Boolean stream) {
        try {
            String response = ragflowChatService.sendAgentMessage(agentId, content, stream);
     ...
     * RAGFlow 简单消息请求数据结构
     */
    public static class RagflowMessageRequest {
