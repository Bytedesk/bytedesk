 ...
 * @LastEditTime: 2025-04-22 11:09:27
...
public class SpringAIVolcengineService extends BaseSpringAIService {
    ...
    /**
     * 根据机器人配置创建动态的OpenAiChatModel（适用于火山引擎）
     * 
     * @param llm 机器人LLM配置
     * @return 配置了特定模型的OpenAiChatModel
     */
    private OpenAiChatModel createDynamicChatModel(RobotLlm llm) {
        if (llm == null || !StringUtils.hasText(llm.getModel()) || !volcengineChatModel.isPresent()) {
            // 如果没有指定模型或设置，使用默认配置
            return volcengineChatModel.orElse(null);
        }

        try {
            // 获取默认模型以复用其API配置
            OpenAiChatModel defaultModel = volcengineChatModel.get();
    ...
    protected void processPrompt(Prompt prompt, MessageProtobuf messageProtobuf) {
        ...
        // 获取适当的模型实例
        OpenAiChatModel chatModel = (llm != null) ? createDynamicChatModel(llm) : volcengineChatModel.orElse(null);
