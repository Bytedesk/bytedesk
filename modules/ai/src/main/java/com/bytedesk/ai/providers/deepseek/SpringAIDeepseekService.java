...
public class SpringAIDeepseekService extends BaseSpringAIService {
    ...
    public SpringAIDeepseekService(
            ...
            @Qualifier("deepseekChatModel") ObjectProvider<DeepSeekChatModel> defaultChatModelProvider,
    ...
    private final DeepSeekChatModel defaultChatModel;
     ...
     */
    private DeepSeekChatOptions createDeepseekOptions(RobotLlm llm) {
    ...
    /**
     * 根据机器人配置创建动态的DeepSeekChatModel
     * 
     * @param llm 机器人LLM配置
     * @return 配置了特定模型的DeepSeekChatModel
     */
    private DeepSeekChatModel createDeepseekChatModel(RobotLlm llm) {
        ...
        try {
            log.info("Creating dynamic Deepseek chat model with provider: {} ({})", provider.getType(),
                    provider.getUid());
            DeepSeekChatOptions options = createDeepseekOptions(llm);
            if (options == null) {
    ...
    protected String processPromptSync(String message, RobotProtobuf robot) {
        DeepSeekChatOptions customOptions = robot != null && robot.getLlm() != null
                ? createDeepseekOptions(robot.getLlm())
                : null;
    ...
    protected String processPromptSync(Prompt prompt, RobotProtobuf robot) {
        ...
        // 获取适当的模型实例
        DeepSeekChatModel chatModel = createDeepseekChatModel(llm);
        try {
            Prompt requestPrompt = prompt;
            DeepSeekChatOptions customOptions = createDeepseekOptions(llm);
    ...
    protected void processPromptSse(Prompt prompt, RobotProtobuf robot, MessageProtobuf messageProtobufQuery,
        ...
        if (llm == null) {
            ...
            sseMessageHelper.sendStreamEndMessage(messageProtobufQuery, messageProtobufReply, emitter, 0, 0, 0, prompt,
                    ...
                    (llm != null && StringUtils.hasText(llm.getTextModel())) ? llm.getTextModel() : LlmDefaults.DEFAULT_DEEPSEEK_CHAT_MODEL);
            return;
        }
