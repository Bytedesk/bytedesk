...
public class ZhipuaiChatService {
    ...
    public ZhipuaiChatService(
            ...
            com.bytedesk.ai.providers.zhipuai.ZhipuaiConfig zhipuaiConfig) {
    ...
    private final com.bytedesk.ai.providers.zhipuai.ZhipuaiConfig zhipuaiConfig;
    ...
    /**
     * 同步聊天 - 参考 ChatCompletionExample
     * 非流式调用，一次性返回完整结果
     */
    public String chatSync(String message) {
        log.info("Zhipuai API sync request - message length: {}", message.length());

        long startTime = System.currentTimeMillis();

        if (isClientUnavailable("chatSync")) {
        ...
        try {
            ...
            ChatCompletionCreateParams request = ChatCompletionCreateParams.builder()
                    ...
                    .messages(messages)
                    .stream(false)
                    .temperature((float) zhipuaiConfig.getTemperature())
                    .topP((float) zhipuaiConfig.getTopP())
                    .maxTokens(zhipuaiConfig.getMaxTokens())
                    .requestId(String.format("sync-%d", System.currentTimeMillis()))
                    .build();

            log.info("Zhipuai API sync invoking model: {}", zhipuaiConfig.getModel());
            ...
            log.info("Zhipuai API sync completed in {}ms", responseTime);
        }
    }

    /**
     * 流式聊天 - 参考 ChatCompletionStreamExample
     * 返回 Flux<String>，每个元素为流式输出的一个 token
     */
    public Flux<String> chatStream(String message) {
        log.info("Zhipuai API stream request - message length: {}", message.length());

        if (isClientUnavailable("chatStream")) {
        ...
        try {
            ...
            ChatCompletionCreateParams request = ChatCompletionCreateParams.builder()
                    ...
                    .topP((float) zhipuaiConfig.getTopP())
                    .maxTokens(zhipuaiConfig.getMaxTokens())
                    .requestId(String.format("stream-%d", System.currentTimeMillis()))
                    .build();

            log.info("Zhipuai API stream invoking model: {}", zhipuaiConfig.getModel());
            ChatCompletionResponse response = client.chat().createChatCompletion(request);
            ...
            if (response.isSuccess() && response.getFlowable() != null) {
                return Flux.from(response.getFlowable())
                        ...
                        .doOnComplete(() -> log.info("Zhipuai API stream completed"))
                        .doOnError(error -> log.error("Zhipuai API stream error: ", error));
            }

            log.error("Zhipuai API stream error: {}", response.getError());
            return Flux.just(buildErrorMessage(response));
        } catch (Exception e) {
            log.error("Zhipuai API stream error: ", e);
    ...
    public List<Double> getEmbedding(String text) {
        if (isClientUnavailable("getEmbedding")) {
        ...
        try {
            ...
            log.error("Zhipuai embedding error", e);
            return new ArrayList<>();
        }
    }
    ...
    public List<List<Double>> getEmbeddings(List<String> texts) {
        if (isClientUnavailable("getEmbeddings")) {
            return new ArrayList<>();
        }
        ...
        try {
            ...
            return result.getData().stream()
                    .map(item -> item.getEmbedding() != null ? item.getEmbedding() : List.<Double>of())
                    .toList();
        } catch (Exception e) {
            log.error("Zhipuai embeddings error", e);
            return new ArrayList<>();
        }
    }

    private String resolveEmbeddingModel() {
        String model = System.getProperty("spring.ai.zhipuai.embedding.options.model");
