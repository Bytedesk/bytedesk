 ...
 * @LastEditTime: 2025-09-25 07:47:09
...
public class SpringAITencentService extends BaseSpringAIService {
    ...
    public SpringAITencentService(
            ...
            @Qualifier("tencentChatModel") ObjectProvider<OpenAiChatModel> defaultChatModelProvider,
    ...
    private OpenAiChatModel createTencentChatModel(RobotLlm llm) {
        ...
        try {
            log.info("Creating Tencent OpenAI chat model with baseUrl: {}, model: {}", provider.getBaseUrl(), llm.getTextModel());
    ...
    RobotLlm llm = robot.getLlm();
        ...
        OpenAiChatModel chatModel = createTencentChatModel(llm);
        if (chatModel == null) {
        sseMessageHelper.handleSseError(new RuntimeException("Tencent service not available"), messageProtobufQuery,
