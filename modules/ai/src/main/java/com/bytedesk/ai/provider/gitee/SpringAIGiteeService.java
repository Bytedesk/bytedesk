 ...
 * @LastEditTime: 2025-09-25 09:24:01
 ...
 */
package com.bytedesk.ai.provider.gitee;
...
public class SpringAIGiteeService extends BaseSpringAIService {
    ...
    private OpenAiChatOptions createDynamicOptions(RobotLlm llm) {
        ...
        try {
            ...
            log.error("Error creating dynamic Gitee options for model {}", llm.getTextModel(), e);
            return null;
        }
    }
    ...
    private OpenAiChatModel createGiteeChatModel(RobotLlm llm) {
        ...
        try {
            ...
            if (options == null) {
                log.warn("Failed to create Gitee options, using default chat model");
                return defaultChatModel;
            }
            ...
            log.error("Failed to create dynamic Gitee chat model for provider {}, using default chat model",
                    provider.getUid(), e);
            return defaultChatModel;
        }
    }
    ...
    //     invokePromptStream(giteeChatModel, requestPrompt).subscribe(
    //             response -> {
    //                 if (response != null) {
    ...
    //                         : "gitee-chat";
    //                 tokenUsageHelper.recordAiTokenUsage(robot, LlmProviderConstants.GITEE, modelType,
    ...
    protected String processPromptSync(Prompt prompt, RobotProtobuf robot) {
        ...
        try {
            ...
            var chatClient = createChatClient(giteeChatModel, requestPrompt, robot);
        ...
        invokePromptStream(chatClient, requestPrompt, conversationId).subscribe(
                ...
                () -> {
                    ...
                    sseMessageHelper.sendStreamEndMessage(messageProtobufQuery, messageProtobufReply, emitter,
                            ...
                            tokenUsage[0].getTotalTokens(), prompt, LlmProviderConstants.GITEE,
