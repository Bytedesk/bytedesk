...
public class ZhipuaiChatController {
    ...
    public ResponseEntity<JsonResult<?>> chatSync(@RequestParam("message") String message) {
        ...
        try {
            String result = zhipuaiChatService.chatSync(message);
    ...
    /**
     * 流式调用 - 参考 ChatCompletionStreamExample
    ...
    public Flux<String> chatStream(@RequestParam("message") String message) {
        ...
        return zhipuaiChatService.chatStream(message)
                .map(chunk -> "data: " + chunk + "\n\n");
    }

    /**
     * SSE调用 - 使用流式接口，逐token推送
    ...
    public SseEmitter chatSse(@RequestParam("message") String message) {
        ...
        zhipuaiChatService.chatStream(message)
                .doOnNext(chunk -> {
                    try {
