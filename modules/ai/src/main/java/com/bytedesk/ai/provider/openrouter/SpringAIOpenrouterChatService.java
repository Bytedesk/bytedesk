...
public class SpringAIOpenrouterChatService extends BaseSpringAIService {
    ...
    public SpringAIOpenrouterChatService(
            @Qualifier("openrouterChatModel") ObjectProvider<OpenAiChatModel> openrouterChatModelProvider,
    ...
    private final OpenAiChatModel openrouterChatModel;
    ...
    //     invokePromptStream(openrouterChatModel, requestPrompt).subscribe(
    //             response -> {
    //                 if (response != null) {
    ...
    protected String processPromptSync(Prompt prompt, RobotProtobuf robot) {
        ...
        try {
            ...
            var chatClient = createChatClient(openrouterChatModel, requestPrompt, robot);
            ...
            long responseTime = System.currentTimeMillis() - startTime;
            String modelType = (llm != null && StringUtils.hasText(llm.getTextModel())) ? llm.getTextModel() : "openrouter-chat";
            tokenUsageHelper.recordAiTokenUsage(robot, LlmProviderConstants.OPENROUTER,
