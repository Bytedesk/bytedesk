...
public class ZhipuaiService extends BaseSpringAIService {
    ...
    private ClientV4 createDynamicClient(RobotLlm llm) {
        ...
        try {
            ...
            log.error("Failed to create dynamic Zhipuai client for provider {}, using default client",
                    provider.getUid(), e);
            return defaultClient;
        }
    }
    ...
    private ChatCompletionRequest createDynamicRequest(RobotLlm llm, String message, boolean stream) {
        ...
        // 添加请求日志
        log.info(
                "Zhipuai API creating request - model: {}, stream: {}, requestId: {}, temperature: {}, message length: {}",
                chatCompletionRequest.getModel(), stream, requestId, chatCompletionRequest.getTemperature(),
                message.length());
    ...
    private ChatCompletionRequest createDynamicRequestFromPrompt(RobotLlm llm, Prompt prompt, boolean stream) {
        ...
        // 添加请求日志
        log.info(
                "Zhipuai API creating request from prompt - model: {}, stream: {}, requestId: {}, temperature: {}, messages count: {}",
                chatCompletionRequest.getModel(), stream, requestId, chatCompletionRequest.getTemperature(),
                messages.size());
    ...
    protected void processPromptWebsocket(Prompt prompt, RobotProtobuf robot, MessageProtobuf messageProtobufQuery,
        ...
        if (chatClient == null) {
            log.error(
                    "Failed to create Zhipuai client and no default client available, cannot process websocket request");
                ...
                mapStreamToAccumulator(response.getFlowable())
                        .doOnNext(accumulator -> {
                            try {
                                ...
                                log.info("Zhipuai API websocket accumulator class: {}",
                                        accumulator.getClass().getName());
                                ...
                                log.info("Zhipuai API websocket delta class: {}",
                        ...
                        .doOnComplete(() -> {
                            ...
                            log.info(
                                    "Zhipuai API websocket recording token usage - prompt: {}, completion: {}, total: {}, model: {}, responseTime: {}ms",
                                    tokenUsage[0].getPromptTokens(), tokenUsage[0].getCompletionTokens(),
                                    tokenUsage[0].getTotalTokens(), modelType, responseTime);
    ...
    protected String processPromptSync(String message, RobotProtobuf robot) {
        ...
        try {
            ...
            String modelType = llm.getTextModel();
            log.info(
                    "Zhipuai API sync recording token usage - prompt: {}, completion: {}, total: {}, model: {}, responseTime: {}ms",
    ...
    protected void processPromptSse(Prompt prompt, RobotProtobuf robot, MessageProtobuf messageProtobufQuery,
        ...
        if (robot == null || robot.getLlm() == null || robot.getLlm().getTextModel() == null) {
            ...
            sendSseMessage(I18Consts.I18N_SERVICE_TEMPORARILY_UNAVAILABLE, robot, messageProtobufQuery,
    ...
    sseMessageHelper.sendStreamStartMessage(messageProtobufQuery, messageProtobufReply, emitter, I18Consts.I18N_THINKING);
        ...
        if (chatClient == null) {
            ...
            sseMessageHelper.handleSseError(new Exception("Failed to create Zhipuai client"),
        ...
        try {
            ...
            if (response.isSuccess()) {
                ...
                mapStreamToAccumulator(response.getFlowable())
                        .doOnNext(accumulator -> {
                            try {
                                ...
                                log.info("Zhipuai API SSE delta class: {}",
                                        delta != null ? delta.getClass().getName() : "null");
                                ...
                                if (delta != null && delta instanceof com.zhipu.oapi.service.v4.model.ChatMessage) {
                                    ...
                                    if (content != null) {
                                        ...
                                        if (!contentStr.isEmpty() && !contentStr.equals("null")) {
                                            sseMessageHelper.sendStreamMessage(messageProtobufQuery,
                                                    messageProtobufReply, emitter, contentStr, null, sourceReferences);
                                        }
                                    }
                                } else {
                                    ...
                                    if (delta != null) {
                                        ...
                                        if (extractedContent != null && !extractedContent.isEmpty()) {
                                            ...
                                            sseMessageHelper.sendStreamMessage(messageProtobufQuery,
                                                    messageProtobufReply, emitter, extractedContent, null,
                                                    sourceReferences);
                                        } else if (!deltaStr.isEmpty() && !deltaStr.equals("null")
                                                && !isEmptyAssistantMessage(deltaStr)) {
                                            // 如果无法提取content，且不是空的助手消息，则发送原始delta字符串
                                            sseMessageHelper.sendStreamMessage(messageProtobufQuery,
                                                    messageProtobufReply, emitter, deltaStr, null, sourceReferences);
                                        }
                                    }
                                }
                                success[0] = true;
                        ...
                        .doOnComplete(() -> {
                            try {
                                ...
                                sseMessageHelper.sendStreamEndMessage(messageProtobufQuery, messageProtobufReply,
                                        emitter,
                                        tokenUsage[0].getPromptTokens(), tokenUsage[0].getCompletionTokens(),
                ...
                sseMessageHelper.handleSseError(
                        new Exception(response.getError() != null ? response.getError().getMessage() : "Unknown error"),
                        messageProtobufQuery, messageProtobufReply, emitter);
            }
        } catch (Exception e) {
            ...
            long responseTime = System.currentTimeMillis() - startTime;
            log.info(
                    "Zhipuai API SSE recording token usage - prompt: {}, completion: {}, total: {}, model: {}, responseTime: {}ms",
                    tokenUsage[0].getPromptTokens(), tokenUsage[0].getCompletionTokens(),
                    tokenUsage[0].getTotalTokens(), modelType, responseTime);
            tokenUsageHelper.recordAiTokenUsage(robot, LlmProviderConstants.ZHIPUAI, modelType,
    ...
    private ChatTokenUsage extractZhipuaiTokenUsage(ModelApiResponse response) {
        try {
            ...
            if (dataStr.contains("usage") || dataStr.contains("tokens")) {
                ...
                if (promptStart == -1)
                    promptStart = dataStr.indexOf("promptTokens=");
                int promptEnd = -1;
                if (promptStart > 0) {
                ...
                if (completionStart == -1)
                    completionStart = dataStr.indexOf("completionTokens=");
                int completionEnd = -1;
                ...
                if (totalStart == -1)
                    totalStart = dataStr.indexOf("totalTokens=");
                int totalEnd = -1;
                if (totalStart > 0) {
                ...
                if (promptStart > 0 && promptEnd > promptStart) {
                    try {
                        ...
                        log.warn("Could not parse prompt_tokens from: {}",
                                dataStr.substring(promptStart + 13, promptEnd));
                    }
                }
                ...
                if (completionStart > 0 && completionEnd > completionStart) {
                    try {
                        ...
                        log.warn("Could not parse completion_tokens from: {}",
                                dataStr.substring(completionStart + 17, completionEnd));
                    }
                }
                ...
                log.info(
                        "Zhipuai API manual token extraction result from string parsing - prompt: {}, completion: {}, total: {}",
                        promptTokens, completionTokens, totalTokens);
    ...
    private ChatTokenUsage estimateTokenUsageFromResponse(ModelApiResponse response) {
        try {
            ...
            log.info(
                    "Zhipuai API estimated tokens - output: {} chars -> {} tokens, estimated prompt: {} tokens, total: {} tokens",
    ...
    private ChatTokenUsage estimateTokenUsageFromMessage(String message) {
        try {
            ...
            log.info(
                    "Zhipuai API estimated token usage from message - input: {} chars -> {} tokens, estimated output: {} tokens, total: {} tokens",
     ...
     */
    private io.reactivex.Flowable<ChatMessageAccumulator> mapStreamToAccumulator(
            io.reactivex.Flowable<ModelData> flowable) {
