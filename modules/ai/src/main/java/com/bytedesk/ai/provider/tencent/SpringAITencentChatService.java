 ...
 * @LastEditTime: 2025-09-25 09:25:34
...
public class SpringAITencentChatService extends BaseSpringAIService {
    ...
    private OpenAiChatOptions createDynamicOptions(RobotLlm llm) {
        ...
        try {
            ...
            log.error("Error creating Tencent options for model {}", llm.getTextModel(), e);
            return null;
        }
    }
    ...
    //     invokePromptStream(tencentChatModel, requestPrompt).subscribe(
    //             response -> {
    //                 if (response != null) {
    ...
    //         tokenUsageHelper.recordAiTokenUsage(robot, LlmProviderConstants.TENCENT, modelType, 
    ...
    protected String processPromptSync(Prompt prompt, RobotProtobuf robot) {
        ...
        try {
            ...
            long responseTime = System.currentTimeMillis() - startTime;
            String modelType = (llm != null && StringUtils.hasText(llm.getTextModel())) ? llm.getTextModel() : "hunyuan-pro";
            tokenUsageHelper.recordAiTokenUsage(robot, LlmProviderConstants.TENCENT,
    ...
    RobotLlm llm = robot.getLlm();
        ...
        if (tencentChatModel == null) {
            sseMessageHelper.handleSseError(new RuntimeException("Tencent service not available"), messageProtobufQuery, messageProtobufReply, emitter);
            return;
        }
