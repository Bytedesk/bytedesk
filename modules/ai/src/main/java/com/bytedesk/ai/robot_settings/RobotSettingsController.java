...
public class RobotSettingsController {
    ...
    @Operation(summary = "获取或创建默认配置", description = "根据组织UID获取或创建默认机器人配置")
    @GetMapping("/default")
    public ResponseEntity<?> getOrCreateDefault(@RequestParam String orgUid) {
        try {
            RobotSettingsEntity settings = robotSettingsService.getOrCreateDefault(orgUid);
    ...
    @Operation(summary = "根据UID查询配置", description = "根据配置UID查询机器人配置详情")
    @GetMapping("/{uid}")
    ...
    @Operation(summary = "创建配置", description = "创建新的机器人配置")
    @PostMapping("/create")
    public ResponseEntity<?> create(@RequestBody RobotSettingsRequest request) {
        try {
            ...
            RobotSettingsEntity created = robotSettingsService.create(settings);
    ...
    @Operation(summary = "更新配置", description = "更新现有的机器人配置")
    @PostMapping("/update")
    public ResponseEntity<?> update(@RequestBody RobotSettingsRequest request) {
        try {
            return robotSettingsService.findByUid(request.getUid())
                    .<ResponseEntity<?>>map(settings -> {
                        ...
                        RobotSettingsEntity updated = robotSettingsService.update(settings);
    ...
    @Operation(summary = "删除配置", description = "根据UID删除机器人配置")
    @DeleteMapping("/{uid}")
    public ResponseEntity<?> delete(@PathVariable String uid) {
        try {
            robotSettingsService.delete(uid);
