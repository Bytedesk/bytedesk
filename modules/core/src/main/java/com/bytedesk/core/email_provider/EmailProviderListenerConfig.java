...
public class EmailProviderListenerConfig {
    ...
    public ListenerStrategy getListenerStrategy(String protocol) {
        if (EmailProviderProtocolEnum.IMAP.name().equals(protocol)) {
            if (idleEnabled) {
                return ListenerStrategy.IDLE;
            } else if (pollingEnabled) {
                return ListenerStrategy.POLLING;
            }
        } else if (EmailProviderProtocolEnum.POP3.name().equals(protocol) || 
                   EmailProviderProtocolEnum.EXCHANGE.name().equals(protocol)) {
            if (pollingEnabled) {
                return ListenerStrategy.POLLING;
            }
        }
