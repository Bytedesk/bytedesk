 ...
 * @LastEditTime: 2025-08-01 21:45:12
...
public class IpAccessRestService extends BaseRestService<IpAccessEntity, IpAccessRequest, IpAccessResponse> {
        ...
        if (entity.getId() != null) {
            ...
            if (refreshedEntity.isPresent()) {
                ...
                if (fresh.getAccessCount() > MAX_REQUESTS_PER_MINUTE) {
                    ipBlackRestService.addToBlackSystem(fresh.getIp());
                }
    ...
    public Boolean isIpBlocked(String ip) {
        // 检查是否在白名单中
        if (ipWhiteRepository.existsByIpAndDeletedFalse(ip)) {
            return false;
        }

        // 检查是否在黑名单中且未过期
        Optional<IpBlackEntity> Black = ipBlackRestService.findByIp(ip);
        if (Black.isPresent() && Black.get().getEndTime().isAfter(BdDateUtils.now())) {
            return true;
        }
                ...
                if (accessOptional.isEmpty()) {
                    ...
                    if (updatedRows == 0) {
                        ...
                        if (updatedAccess.isPresent() && updatedAccess.get().getAccessCount() > MAX_REQUESTS_PER_MINUTE) {
                            ipBlackRestService.addToBlackSystem(ip);
                        }
                    }
                }
