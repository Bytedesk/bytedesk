...
public class WorkflowNodeFactory {
    ...
    public static WorkflowBaseNode createNode(String nodeType) {
        ...
        switch (type) {
            ...
            case PARENT:
                return WorkflowParentNode.builder().type(nodeType).build();
            case FORM:
                return WorkflowFormNode.builder().type(nodeType).build();
            default:
    ...
    public static WorkflowBaseNode parseNode(JSONObject nodeObject) {
        if (nodeObject == null) {
            return null;
        }
        WorkflowNodeTypeEnum type = WorkflowNodeTypeEnum.fromValue(nodeObject.getString("type"));
        String json = nodeObject.toJSONString();
        switch (type) {
            case START:
                return WorkflowStartNode.fromJson(json);
            case END:
                return WorkflowEndNode.fromJson(json);
            case CONDITION:
                return WorkflowConditionNode.fromJson(json);
            case LOOP:
                return WorkflowLoopNode.fromJson(json);
            case LLM:
                return WorkflowLLMNode.fromJson(json);
            case TEXT:
                return WorkflowTextNode.fromJson(json);
            case COMMENT:
                return WorkflowCommentNode.fromJson(json);
            case GROUP:
                return WorkflowGroupNode.fromJson(json);
            case PARENT:
                return WorkflowParentNode.fromJson(json);
            case FORM:
                return WorkflowFormNode.fromJson(json);
            default:
