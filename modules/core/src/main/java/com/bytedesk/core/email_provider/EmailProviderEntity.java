...
@AllArgsConstructor
@NoArgsConstructor
@EntityListeners({EmailProviderEntityListener.class})
@Table(name = "bytedesk_core_email_provider")
public class EmailProviderEntity extends BaseEntity {
    ...
    private String provider = EmailProviderProviderEnum.QQ.name();

    // 用途类型：在线客服接待/工单客服 等
    @Builder.Default
    @Column(name = "email_type")
    private String type = EmailProviderTypeEnum.TICKET.name();

    // 邮箱协议类型：IMAP/POP3/SMTP/EXCHANGE
    @Builder.Default
    @Column(name = "email_protocol")
    private String protocol = EmailProviderProtocolEnum.IMAP.name();
    ...
    // 连接状态：连接成功/连接失败
    @Builder.Default
    private String connectionStatus = EmailProviderConnectionStatusEnum.DISCONNECTED.name();
