...
public class ApnsTokenRestController extends BaseRestController<ApnsTokenRequest, ApnsTokenRestService> {
    ...
    @ActionAnnotation(title = I18Consts.I18N_APNS_TOKEN, action = "test_push", description = "test push notification to apns token")
    @Operation(summary = "Test Push APNs Token", description = "Send a test push notification via the specified APNs token")
    @PreAuthorize(ApnsTokenPermissions.HAS_APNS_TOKEN_UPDATE)
    @PostMapping("/test_push")
    public ResponseEntity<?> testPush(@RequestBody ApnsTokenRequest request) {

        String uid = request.getUid();
        if (uid == null || uid.isBlank()) {
            return ResponseEntity.badRequest().body(JsonResult.error("token uid is required"));
        }

        ApnsTokenEntity apnsToken = apnsTokenRestService.findByUid(uid)
                .orElse(null);
        if (apnsToken == null) {
            return ResponseEntity.badRequest().body(JsonResult.error("apns token not found"));
        }

        String userUid = apnsToken.getUserUid();
        if (userUid == null || userUid.isBlank()) {
            return ResponseEntity.badRequest().body(JsonResult.error("apns token has no associated user"));
        }

        apnsPushService.pushNotificationToUser(
                userUid,
                "测试推送",
                "这是一条来自管理后台的测试推送消息",
                null);

        return ResponseEntity.ok(JsonResult.success("test push sent"));
    }
