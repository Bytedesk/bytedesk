...
public class EmailPushRestService extends BaseRestServiceWithExport<EmailPushEntity, EmailPushRequest, EmailPushResponse, EmailPushExcel> {
    ...
    @Cacheable(value = "email_push", key = "#name + '_' + #orgUid + '_' + #type", unless="#result==null")
    public Optional<EmailPushEntity> findByNameAndOrgUidAndType(String name, String orgUid, String type) {
        return emailPushRepository.findByNameAndOrgUidAndTypeAndDeletedFalse(name, orgUid, type);
    }
    ...
    private EmailPushResponse createInternal(EmailPushRequest request, boolean skipPermissionCheck) {
        ...
        if (StringUtils.hasText(request.getName()) && StringUtils.hasText(request.getOrgUid()) && StringUtils.hasText(request.getType())) {
            Optional<EmailPushEntity> email_push = findByNameAndOrgUidAndType(request.getName(), request.getOrgUid(), request.getType());
            if (email_push.isPresent()) {
                return convertToResponse(email_push.get());
            }
        }
