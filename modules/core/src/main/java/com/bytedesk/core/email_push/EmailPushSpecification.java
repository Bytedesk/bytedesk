...
@Slf4j
public class EmailPushSpecification extends BaseSpecification<EmailPushEntity, EmailPushRequest> {
    
    public static Specification<EmailPushEntity> search(EmailPushRequest request, AuthService authService) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            // 使用带层级过滤的基础条件
            predicates.addAll(getBasicPredicatesWithLevel(root, criteriaBuilder, request, authService, EmailPushPermissions.MODULE_NAME));
            if (StringUtils.hasText(request.getSender())) {
                predicates.add(criteriaBuilder.like(root.get("sender"), "%" + request.getSender() + "%"));
            }
