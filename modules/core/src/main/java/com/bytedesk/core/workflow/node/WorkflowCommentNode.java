...
public class WorkflowCommentNode extends WorkflowBaseNode {
    ...
    public WorkflowNodeExecutionResult execute(WorkflowExecutionContext context) {
        ...
        output.put("note", note != null ? note : (getData() != null ? getData().getNote() : null));
        output.put("size", size != null ? size : (getData() != null ? getData().getSize() : null));
        return WorkflowNodeExecutionResult.success("Comment node skipped", context.findNextNodeId(getId()), output);
    }
