...
public class StompController {
    ...
    public void message(Principal principal,
        ...
        try {
            ...
            sendErrorToSession(sessionId, buildErrorPayload(message, exception));
        }
    }
    ...
    private void sendErrorToSession(String sessionId, StompErrorPayload payload) {
        if (sessionId == null || sessionId.isBlank()) {
            log.warn("skip stomp error push because sessionId is blank, payload: {}", payload);
            return;
        }
    ...
    private StompErrorPayload buildErrorPayload(String message, RuntimeException exception) {
        ...
        try {
            ...
            log.debug("failed to parse stomp message uid from payload", parseException);
        }
