...
@Slf4j
public class MenuSpecification extends BaseSpecification<MenuEntity, MenuRequest> {
    
    public static Specification<MenuEntity> search(MenuRequest request, AuthService authService) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            if (StringUtils.hasText(request.getLink())) {
                predicates.add(criteriaBuilder.like(root.get("link"), "%" + request.getLink() + "%"));
            }
            // icon
            if (StringUtils.hasText(request.getIcon())) {
                predicates.add(criteriaBuilder.equal(root.get("icon"), request.getIcon()));
            }
            // type
            ...
            if (StringUtils.hasText(request.getParentUid())) {
                predicates.add(criteriaBuilder.equal(root.get("parentUid"), request.getParentUid()));
            }
            // enabled
            ...
            if (request.getOpenInNewWindow() != null) {
                predicates.add(criteriaBuilder.equal(root.get("openInNewWindow"), request.getOpenInNewWindow()));
            }
            // 
