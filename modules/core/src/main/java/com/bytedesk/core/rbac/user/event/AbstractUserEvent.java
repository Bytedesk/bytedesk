...
public abstract class AbstractUserEvent extends ApplicationEvent {
    ...
    protected AbstractUserEvent(Object source, UserEntity user) {
        super(source);
    ...
    private UserEntity snapshot(UserEntity source) {
        if (source == null) {
            return null;
        }
        try {
            ...
            throw new IllegalStateException("Failed to snapshot user " + source.getUid(), ex);
        }
    }
}
