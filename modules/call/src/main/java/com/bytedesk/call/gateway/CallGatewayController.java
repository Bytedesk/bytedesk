...
public class CallGatewayController {
     ...
     */
    @PostMapping
    public ResponseEntity<JsonResult<?>> createGateway(@Valid @RequestBody CallGatewayRequest request) {
        try {
            CallGatewayEntity gateway = gatewayService.createGateway(
            ...
            CallGatewayResponse response = CallGatewayResponse.fromEntitySafe(gateway);
    ...
    public ResponseEntity<JsonResult<?>> getGateway(@PathVariable Long id) {
        Optional<CallGatewayEntity> gateway = gatewayService.findById(id);
        if (gateway.isPresent()) {
            CallGatewayResponse response = CallGatewayResponse.fromEntitySafe(gateway.get());
    ...
    public ResponseEntity<JsonResult<?>> getGatewayByName(@PathVariable String gatewayName) {
        Optional<CallGatewayEntity> gateway = gatewayService.findByGatewayName(gatewayName);
        if (gateway.isPresent()) {
            CallGatewayResponse response = CallGatewayResponse.fromEntitySafe(gateway.get());
            return ResponseEntity.ok(JsonResult.success("获取网关详情成功", response));
        } else {
    ...
    public ResponseEntity<JsonResult<?>> getGateways(
        ...
        Page<CallGatewayEntity> gatewayPage = gatewayService.findAll(pageable);
        Page<CallGatewayResponse> responsePage = gatewayPage.map(CallGatewayResponse::fromEntitySafe);
    ...
    public ResponseEntity<JsonResult<?>> getEnabledGateways() {
        List<CallGatewayEntity> gateways = gatewayService.findEnabledGateways();
        List<CallGatewayResponse> responses = gateways.stream()
                .map(CallGatewayResponse::fromEntitySafe)
    ...
    public ResponseEntity<JsonResult<?>> getOnlineGateways() {
        List<CallGatewayEntity> gateways = gatewayService.findOnlineGateways();
        List<CallGatewayResponse> responses = gateways.stream()
                .map(CallGatewayResponse::fromEntitySafe)
                .collect(Collectors.toList());
    ...
    public ResponseEntity<JsonResult<?>> getGatewaysByStatus(@PathVariable String status) {
        List<CallGatewayEntity> gateways = gatewayService.findByStatus(status);
        List<CallGatewayResponse> responses = gateways.stream()
                .map(CallGatewayResponse::fromEntitySafe)
                .collect(Collectors.toList());
    ...
    public ResponseEntity<JsonResult<?>> updateGateway(
            ...
            @Valid @RequestBody CallGatewayRequest request) {
        try {
            CallGatewayEntity gateway = gatewayService.updateGateway(
                    id,
                    request.getDescription(),
                    request.getProxy(),
                    request.getUsername(),
