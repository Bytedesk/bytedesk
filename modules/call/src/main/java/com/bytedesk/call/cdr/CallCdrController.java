...
public class CallCdrController {
    ...
    public ResponseEntity<?> getCdrList(
        ...
        try {
            ...
            // Page<CallCdrEntity> cdrPage = cdrService.findAllCdr(pageable);
            
            return ResponseEntity.ok(JsonResult.success());
        } catch (Exception e) {
            log.error("获取Call CDR列表失败", e);
    ...
    public ResponseEntity<?> getCdrById(@PathVariable Long id) {
        try {
            Optional<CallCdrEntity> cdr = cdrService.findById(id);
            if (cdr.isPresent()) {
    ...
    public ResponseEntity<?> getCdrByUid(@PathVariable String uid) {
        try {
            Optional<CallCdrEntity> cdr = cdrService.findByUid(uid);
            if (cdr.isPresent()) {
                return ResponseEntity.ok(JsonResult.success(cdr.get()));
            } else {
                return ResponseEntity.notFound().build();
    ...
    public ResponseEntity<?> getCdrByCallerNumber(
        ...
        try {
            List<CallCdrEntity> cdrList = cdrService.findByCallerNumber(callerNumber);
            return ResponseEntity.ok(JsonResult.success(cdrList));
        } catch (Exception e) {
            log.error("根据主叫号码查询Call CDR失败", e);
            return ResponseEntity.badRequest().body(JsonResult.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getCdrByDestinationNumber(
        ...
        try {
            List<CallCdrEntity> cdrList = cdrService.findByDestinationNumber(destinationNumber);
            return ResponseEntity.ok(JsonResult.success(cdrList));
        } catch (Exception e) {
            log.error("根据被叫号码查询Call CDR失败", e);
            return ResponseEntity.badRequest().body(JsonResult.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getRecentCalls(
        ...
        try {
            ...
            Page<CallCdrEntity> recentCalls = cdrService.getRecentCalls(pageable);
            return ResponseEntity.ok(JsonResult.success(recentCalls));
        } catch (Exception e) {
            log.error("获取最近Call通话记录失败", e);
            return ResponseEntity.badRequest().body(JsonResult.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getAnsweredCalls() {
        try {
            List<CallCdrEntity> answeredCalls = cdrService.getAnsweredCalls();
            return ResponseEntity.ok(JsonResult.success(answeredCalls));
        } catch (Exception e) {
            log.error("获取已应答Call通话记录失败", e);
            return ResponseEntity.badRequest().body(JsonResult.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getRecordedCalls() {
        try {
            List<CallCdrEntity> recordedCalls = cdrService.getRecordedCalls();
            return ResponseEntity.ok(JsonResult.success(recordedCalls));
        } catch (Exception e) {
            log.error("获取有录音的Call通话记录失败", e);
            return ResponseEntity.badRequest().body(JsonResult.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getCallStatistics(
        ...
        try {
            CallCdrService.CallStatistics statistics = cdrService.getCallStatistics(startTime, endTime);
            return ResponseEntity.ok(JsonResult.success(statistics));
        } catch (Exception e) {
            log.error("获取Call通话统计失败", e);
            return ResponseEntity.badRequest().body(JsonResult.error(e.getMessage()));
        }
    }
