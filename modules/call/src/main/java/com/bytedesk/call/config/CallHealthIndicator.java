 ...
 * @LastEditTime: 2025-07-04 10:31:44
...
public class CallHealthIndicator implements HealthIndicator {
    ...
    private String getCurrentTime() {
        return BdDateUtils.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
    }
    ...
    //             if (response.contains("rude-rejection")) {
    //                 return "REJECTED - Call ACL configuration blocks connection: " + response;
    //             } else if (response.contains("auth/request")) {
    //                 return "SUCCESS - Call ESL server is responding properly";
    //             } else {
    ...
    //     } catch (IOException e) {
    //         if (e.getMessage().contains("Connection refused")) {
    //             return "CONNECTION_REFUSED - Call service may not be running";
    //         } else if (e.getMessage().contains("timeout")) {
    ...
    //         return "ACL配置问题 - 需要修改Call的event_socket.conf.xml文件，移除或修改apply-inbound-acl参数";
    //     } else if (eslStatus.contains("CONNECTION_REFUSED")) {
    //         return "服务未运行 - 检查Call服务状态: systemctl status freeswitch";
    //     } else if (eslStatus.contains("TIMEOUT")) {
    //         return "网络问题 - 检查防火墙和网络连接";
    //     } else {
    //         return "未知问题 - 查看Call日志: tail -f /usr/local/freeswitch/log/freeswitch.log";
    //     }
    // }
