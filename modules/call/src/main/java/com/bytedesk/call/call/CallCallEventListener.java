...
public class CallCallEventListener {
     ...
     */
    @EventListener
    public void handleCallStartEvent(CallCallStartEvent event) {
     ...
     */
    @EventListener
    public void handleCallAnsweredEvent(CallCallAnsweredEvent event) {
        log.info("接收到通话应答事件: UUID {}", event.getUuid());
        
        try {
            callService.handleCallAnswered(event.getUuid());
            
            // 额外的数据库操作已在CallEventListener中处理
     ...
     */
    @EventListener
    public void handleCallHangupEvent(CallCallHangupEvent event) {
        ...
        try {
            ...
            // 额外的CDR更新已在CallEventListener中处理
     ...
     */
    @EventListener
    public void handleDtmfEvent(CallDtmfEvent event) {
