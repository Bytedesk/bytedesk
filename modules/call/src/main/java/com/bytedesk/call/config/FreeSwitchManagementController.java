...
/**
 * FreeSwitch管理API
 * 提供FreeSwitch连接状态查询和诊断功能
 */
@Slf4j
@RestController
@RequestMapping("/api/v1/freeswitch")
@RequiredArgsConstructor
@Tag(name = "FreeSwitch Management", description = "FreeSwitch连接管理和诊断API")
@ConditionalOnProperty(name = "bytedesk.freeswitch.enabled", havingValue = "true", matchIfMissing = false)
public class FreeSwitchManagementController {

    private final FreeSwitchProperties freeSwitchProperties;
    private final FreeSwitchConnectionTester connectionTester;
    private final FreeSwitchHealthIndicator healthIndicator;

    /**
     * 获取FreeSwitch配置信息
     */
    @GetMapping("/config")
    @Operation(summary = "获取FreeSwitch配置信息", description = "返回当前的FreeSwitch连接配置")
     ...
     */
    @GetMapping("/status")
    @Operation(summary = "获取FreeSwitch连接状态", description = "返回详细的连接状态和诊断信息")
    public ResponseEntity<?> getStatus() {
        try {
            FreeSwitchHealthIndicator.ConnectionStatus status = healthIndicator.getDetailedStatus();
            return ResponseEntity.ok(status);
        } catch (Exception e) {
            log.error("获取FreeSwitch状态失败", e);
            return ResponseEntity.internalServerError()
    ...
    @Operation(summary = "测试FreeSwitch连接", description = "主动测试与FreeSwitch的连接并返回结果")
    public ResponseEntity<?> testConnection() {
        try {
            log.info("管理员触发FreeSwitch连接测试");
    ...
    @Operation(summary = "获取故障排除指南", description = "返回FreeSwitch连接问题的排除指南")
