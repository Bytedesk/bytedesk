...
@Slf4j
public class EslEventSpecification extends BaseSpecification<EslEventEntity, EslEventRequest> {
    
    public static Specification<EslEventEntity> search(EslEventRequest request, AuthService authService) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            // 使用带层级过滤的基础条件
            predicates.addAll(getBasicPredicatesWithLevel(root, criteriaBuilder, request, authService, EslEventPermissions.MODULE_NAME));
            // name
            ...
            // eventSubclass
            if (StringUtils.hasText(request.getEventSubclass())) {
                predicates.add(criteriaBuilder.like(root.get("eventSubclass"), "%" + request.getEventSubclass() + "%"));
            }
            // uniqueId
            if (StringUtils.hasText(request.getUniqueId())) {
                predicates.add(criteriaBuilder.like(root.get("uniqueId"), "%" + request.getUniqueId() + "%"));
            }
            // callerNumber
            if (StringUtils.hasText(request.getCallerNumber())) {
                predicates.add(criteriaBuilder.like(root.get("callerNumber"), "%" + request.getCallerNumber() + "%"));
            }
            // destinationNumber
