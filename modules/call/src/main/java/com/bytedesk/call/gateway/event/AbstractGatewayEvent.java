...
/**
 * Ensures every gateway-related application event carries a detached snapshot so
...
public abstract class AbstractGatewayEvent extends ApplicationEvent {
    ...
    protected AbstractGatewayEvent(Object source, GatewayEntity gateway) {
        super(source);
    ...
    private GatewayEntity snapshot(GatewayEntity source) {
        if (source == null) {
            return null;
        }
        try {
            ...
            throw new IllegalStateException("Failed to snapshot gateway " + source.getUid(), ex);
        }
    }
}
