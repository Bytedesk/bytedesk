...
@Slf4j
public class CustomerCompanySpecification extends BaseSpecification<CustomerCompanyEntity, CustomerCompanyRequest> {
    
    public static Specification<CustomerCompanyEntity> search(CustomerCompanyRequest request, AuthService authService) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            // 使用带层级过滤的基础条件
            predicates.addAll(getBasicPredicatesWithLevel(root, criteriaBuilder, request, authService, CustomerCompanyPermissions.MODULE_NAME));
            // name
            ...
            if (StringUtils.hasText(request.getWebsite())) {
                predicates.add(criteriaBuilder.like(root.get("website"), "%" + request.getWebsite() + "%"));
            }
            // ownerUserUid
            if (StringUtils.hasText(request.getOwnerUserUid())) {
                predicates.add(criteriaBuilder.equal(root.get("ownerUserUid"), request.getOwnerUserUid()));
            }
            // type
