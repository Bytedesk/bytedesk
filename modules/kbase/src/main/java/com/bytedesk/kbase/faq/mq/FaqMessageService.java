 ...
 * @LastEditTime: 2025-05-19 20:27:44
...
public class FaqMessageService {
        ...
        try {
            ...
            org.springframework.jms.core.MessagePostProcessor postProcessor = new org.springframework.jms.core.MessagePostProcessor() {
                ...
                public jakarta.jms.Message postProcessMessage(jakarta.jms.Message jmsMessage) throws jakarta.jms.JMSException {
                    ...
                    // 设置消息分组，确保相同FAQ的消息按顺序处理
                    jmsMessage.setStringProperty("JMSXGroupID", "faq-" + faqUid);
    ...
    public void batchSendToIndexQueue(Iterable<String> faqUids) {
        try {
            ...
            log.info("批量发送FAQ到索引队列完成，共发送了{}条消息，耗时{}ms", count, duration);
        } catch (Exception e) {
    ...
    private void processBatch(java.util.List<String> batch, int totalCount) {
        ...
        log.debug("处理第{}批FAQ索引请求，共{}条", (totalCount / batchSize) + 1, batchSize);
