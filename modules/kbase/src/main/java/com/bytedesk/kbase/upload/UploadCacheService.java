 ...
 * @LastEditTime: 2024-08-20 12:47:28
...
public class UploadCacheService {
    ...
    // 创建一个缓存实例，设置过期时间为5天
    Cache<String, List<String>> uploadCache = Caffeine.newBuilder()
    ...
    String defaultCacheKey = "uploadList";
    ...
    public void push(String listKey, String messageJSON) {
        List<String> cachedList = uploadCache.getIfPresent(listKey);
        if (cachedList == null) {
            // 如果缓存中没有找到对应的键，则使用load方法初始化
            cachedList = new ArrayList<>();
        }
        cachedList.add(messageJSON);
        uploadCache.put(listKey, cachedList);
    }
    ...
    public void remove(String listKey) {
        uploadCache.invalidate(listKey);
    }

    public void clear() {
        uploadCache.invalidateAll();
    }
