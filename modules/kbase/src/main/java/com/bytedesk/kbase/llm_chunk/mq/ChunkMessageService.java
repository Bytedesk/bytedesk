...
public class ChunkMessageService {
     ...
     * 发送单个Chunk到索引队列
     * 
     * @param chunkUid Chunk的唯一标识
     */
    public void sendToIndexQueue(String chunkUid) {
        sendToIndexQueue(chunkUid, null);
    }
    ...
    public void sendToIndexQueue(String chunkUid, String fileUid) {
        try {
            ...
            ChunkIndexMessage message = ChunkIndexMessage.builder()
                    .chunkUid(chunkUid)
                    .operationType("index")
            ...
            org.springframework.jms.core.MessagePostProcessor postProcessor = jmsMessage -> {
                ...
                // 设置消息分组，确保相同文件的Chunk按顺序处理
                if (fileUid != null) {
                    jmsMessage.setStringProperty("JMSXGroupID", "file-" + fileUid);
                } else {
                    jmsMessage.setStringProperty("JMSXGroupID", "chunk-" + chunkUid);
                }
            ...
            log.error("发送Chunk索引消息失败: {}", e.getMessage(), e);
        }
    }
    ...
    public void sendToDeleteQueue(String chunkUid) {
        sendToDeleteQueue(chunkUid, null);
    }
    ...
    public void sendToDeleteQueue(String chunkUid, String fileUid) {
        try {
            ...
            log.error("发送Chunk删除消息失败: {}", e.getMessage(), e);
        }
    }
    ...
    public void batchSendToIndexQueue(List<String> chunkUids, String fileUid) {
        ...
        try {
            log.info("批量发送Chunk到索引队列，共{}个chunks，文件: {}", chunkUids.size(), fileUid);
            
            int batchSize = Math.min(10, chunkUids.size()); // 动态调整批处理大小，最大10个
            String batchId = "batch-" + System.currentTimeMillis() + "-" + random.nextInt(1000);
            long startTime = System.currentTimeMillis();
            int totalCount = chunkUids.size();
            
            // 分批处理
            for (int i = 0; i < totalCount; i += batchSize) {
                int endIndex = Math.min(i + batchSize, totalCount);
                List<String> batch = chunkUids.subList(i, endIndex);
                
                log.debug("处理第{}批Chunk，范围: {}-{}", (i / batchSize) + 1, i, endIndex - 1);
            ...
            log.info("批量发送Chunk到索引队列完成，共发送了{}条消息，耗时{}ms，批次ID: {}", 
    ...
    public void batchSendToDeleteQueue(List<String> chunkUids, String fileUid) {
        if (chunkUids == null || chunkUids.isEmpty()) {
            log.warn("批量删除的Chunk列表为空");
            return;
        }
        
        try {
            log.info("批量发送Chunk删除请求，共{}个chunks，文件: {}", chunkUids.size(), fileUid);
            
            String batchId = "delete-batch-" + System.currentTimeMillis() + "-" + random.nextInt(1000);
            
            for (int i = 0; i < chunkUids.size(); i++) {
                String chunkUid = chunkUids.get(i);
            ...
            log.info("批量删除消息发送完成，共{}条消息，批次ID: {}", chunkUids.size(), batchId);
        } catch (Exception e) {
            log.error("批量发送Chunk删除消息失败: {}", e.getMessage(), e);
        }
    }
    ...
    public void sendChunkCompleteNotification(String chunkUid, String fileUid, String status, String processType) {
        try {
            ...
            log.debug("已发送chunk完成通知: chunk={}, file={}, status={}, type={}", 
                    chunkUid, fileUid, status, processType);
        } catch (Exception e) {
            log.error("发送chunk完成通知失败: {}, 错误: {}", chunkUid, e.getMessage(), e);
        }
    }
}
