 ...
 * @LastEditTime: 2025-09-11 15:54:42
...
@Table(name = "bytedesk_kbase_llm_image")
public class ImageEntity extends BaseEntity {

    private String name;

    // image 之后可能不需要这么长的 content，待优化
    @Column(columnDefinition = TypeConsts.COLUMN_TYPE_TEXT)
    private String content;

    @Builder.Default
    @Column(name = "image_type")
    private String type = ImageTypeEnum.FILE.name();
    ...
    @Builder.Default
    private String elasticStatus = ImageStatusEnum.NEW.name();

    @Builder.Default
    private String vectorStatus = ImageStatusEnum.NEW.name();
    ...
    // set Success
    public ImageEntity setElasticSuccess() {
        this.setElasticStatus(ImageStatusEnum.SUCCESS.name());
        return this;
    }

    // set Error
    public ImageEntity setElasticError() {
        this.setElasticStatus(ImageStatusEnum.ERROR.name());
        return this;
    }

    // set vector Success
    public ImageEntity setVectorSuccess() {
        this.setVectorStatus(ImageStatusEnum.SUCCESS.name());
        return this;
    }

    // set vector Error
    public ImageEntity setVectorError() {
        this.setVectorStatus(ImageStatusEnum.ERROR.name());
        return this;
    }
     ...
     * @param request ImageRequest 请求对象
     * @return 如果关键内容有变化返回 true，否则返回 false
     */
    public Boolean hasChanged(ImageRequest request) {
        // 比较名称是否变化
