 ...
 */
public abstract class AbstractFileEvent extends ApplicationEvent {

    private static final long serialVersionUID = 1L;

    private final FileEntity file;

    protected AbstractFileEvent(Object source, FileEntity file) {
        super(source);
    ...
    private FileEntity snapshot(FileEntity source) {
        if (source == null) {
            return null;
        }
        try {
            ...
            throw new IllegalStateException("Failed to snapshot file " + source.getUid(), ex);
        }
    }
}
