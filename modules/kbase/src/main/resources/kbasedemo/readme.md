...
```bash
git clone https://github.com/Bytedesk/bytedesk.git && cd bytedesk/deploy/docker && ./start.sh all
```

### 或者 仅启动中间件（源码启动推荐）

```bash
git clone https://github.com/Bytedesk/bytedesk.git && cd bytedesk/deploy/docker && ./start.sh middleware
# 运行模型
...
- 修改 `deploy/docker/compose/compose-bytedesk.yaml` 文件
