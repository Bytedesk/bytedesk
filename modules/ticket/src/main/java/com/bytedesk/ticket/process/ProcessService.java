...
public class ProcessService {
    ...
    public List<ProcessDefinitionResponse> query(ProcessRequest request) {
        ...
        // 先查询已部署的流程定义实体
        List<ProcessEntity> deployedProcesses = ProcessRepository.findByOrgUidAndStatus(orgUid, ProcessStatusEnum.DEPLOYED.name());
        ...
        List<ProcessDefinitionResponse> ProcessResponses = processList.stream()
    ...
    public ProcessDefinitionResponse deploy(ProcessRequest request) {
        Optional<ProcessEntity> Process = ProcessRepository.findByUid(request.getUid());
        if (!Process.isPresent()) {
            throw new RuntimeException("流程定义不存在" + request.getUid());
        }
        String orgUid = Process.get().getOrgUid();
        String bpmnXml = Process.get().getSchema();
        ...
        // 更新流程定义
        // Process.get().setDeployed(true);
        Process.get().setStatus(ProcessStatusEnum.DEPLOYED.name());
        Process.get().setDeploymentId(deployment.getId());
    ...
    public List<ProcessDefinitionResponse> undeploy(ProcessRequest request) {
        ...
        // 获取部署ID
        String deploymentId = Process.get().getDeploymentId();
        ...
        try {
            ...
            // 更新实体状态
            // Process.get().setDeployed(false);
            Process.get().setStatus(ProcessStatusEnum.DRAFT.name());
            Process.get().setDeploymentId(null);
            ProcessRepository.save(Process.get());
        ...
        // 转换为 DTO 列表返回
        List<ProcessDefinitionResponse> ProcessResponses = remainingProcesses.stream()
            .map(def -> ProcessDefinitionResponse.builder()
                .id(def.getId())
                .key(def.getKey())
                .name(def.getName())
        ...
        return ProcessResponses;  // 返回删除前的流程定义列表
    }
