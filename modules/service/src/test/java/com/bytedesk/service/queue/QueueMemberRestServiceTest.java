...
class QueueMemberRestServiceTest {
    ...
    void enqueueAssignsSequentialQueueNumbers() {
        ...
        when(queueMemberRepository.findFirstByThreadUidAndDeletedFalseAndStatusInForUpdate(eq("thread-1"), anyList()))
                .thenReturn(Optional.empty());
        when(queueMemberRepository.findFirstByThreadUidAndDeletedFalseAndStatusInForUpdate(eq("thread-2"), anyList()))
                .thenReturn(Optional.empty());
        ...
        verify(queueMemberRepository, atLeastOnce()).findIdleBefore(any());
                verify(queueAuditLogger, times(2)).logQueueJoin(any(QueueMemberEntity.class), any(ThreadEntity.class), eq(agentQueue), eq(QueueTypeEnum.AGENT));
    }
    ...
    void enqueueReturnsExistingMemberWhenDuplicateThread() {
        ...
        when(queueMemberRepository.findIdleBefore(any())).thenReturn(Collections.emptyList());
        when(queueMemberRepository.findFirstByThreadUidAndDeletedFalseAndStatusInForUpdate(eq("thread-dup"), anyList()))
    ...
    void findAgentQueueMembersReturnsPagedSnapshot() {
        ...
        try (MockedStatic<ServiceConvertUtils> mocked = Mockito.mockStatic(ServiceConvertUtils.class)) {
            ...
            Page<QueueMemberResponse> result = queueMemberRestService.findAgentQueueMembers("queue-uid", pageable);
