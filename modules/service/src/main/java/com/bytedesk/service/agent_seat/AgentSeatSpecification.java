...
@Slf4j
public class AgentSeatSpecification extends BaseSpecification<AgentSeatEntity, AgentSeatRequest> {
    
    public static Specification<AgentSeatEntity> search(AgentSeatRequest request, AuthService authService) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            // 使用带层级过滤的基础条件
            predicates.addAll(getBasicPredicatesWithLevel(root, criteriaBuilder, request, authService, AgentSeatPermissions.MODULE_NAME));
            // seatNo
            if (StringUtils.hasText(request.getSeatNo())) {
                predicates.add(criteriaBuilder.like(root.get("seatNo"), "%" + request.getSeatNo() + "%"));
            }
            // source
