...
@RestController
@RequestMapping("/api/v1/group_invite")
@AllArgsConstructor
public class GroupInviteRestController extends BaseRestController<GroupInviteRequest> {

    private final GroupInviteRestService group_inviteService;

    @PreAuthorize(RolePermissions.ROLE_ADMIN)
    @Override
    public ResponseEntity<?> queryByOrg(GroupInviteRequest request) {
        
        Page<GroupInviteResponse> group_invites = group_inviteService.queryByOrg(request);

        return ResponseEntity.ok(JsonResult.success(group_invites));
    }

    @Override
    public ResponseEntity<?> queryByUser(GroupInviteRequest request) {
        
        Page<GroupInviteResponse> group_invites = group_inviteService.queryByUser(request);
    ...
    public ResponseEntity<?> create(GroupInviteRequest request) {
        ...
        GroupInviteResponse group_invite = group_inviteService.create(request);

        return ResponseEntity.ok(JsonResult.success(group_invite));
    }

    @Override
    public ResponseEntity<?> update(GroupInviteRequest request) {
        
        GroupInviteResponse group_invite = group_inviteService.update(request);
    ...
    public ResponseEntity<?> delete(GroupInviteRequest request) {
        ...
        group_inviteService.delete(request);
