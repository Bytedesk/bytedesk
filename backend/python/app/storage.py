...
assistant_hash_keys = ["assistant_id",
                       "name", "config", "updated_at", "public"]
...
def list_assistants(user_id: str) -> List[Assistant]:
    ...
    ids = [orjson.loads(id)
           for id in client.smembers(assistants_list_key(user_id))]
...
def get_assistant(user_id: str, assistant_id: str) -> Assistant | None:
    ...
    client = _get_redis_client()
    values = client.hmget(assistant_key(
        user_id, assistant_id), *assistant_hash_keys)
...
def put_assistant(
    ...
    with client.pipeline() as pipe:
        ...
        if public:
            pipe.sadd(assistants_list_key(public_user_id),
                      orjson.dumps(assistant_id))
            pipe.hset(assistant_key(public_user_id, assistant_id),
                      mapping=_dump(saved))
        pipe.execute()
    return saved
...
def list_threads(user_id: str) -> List[ThreadWithoutUserId]:
    ...
    client = _get_redis_client()
    ids = [orjson.loads(id)
           for id in client.smembers(threads_list_key(user_id))]
    with client.pipeline() as pipe:
        for id in ids:
...
if __name__ == "__main__":
    ...
    put_assistant("123", "i-am-a-test", name="Test Agent",
                  config={"tags": ["hello"]})
