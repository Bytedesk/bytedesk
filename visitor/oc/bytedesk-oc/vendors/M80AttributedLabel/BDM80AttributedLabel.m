...
@interface BDM80AttributedLabel ()
@property (nonatomic,strong)    NSMutableAttributedString   *attributedString;
@property (nonatomic,strong)    NSMutableArray              *attachments;
@property (nonatomic,strong)    NSMutableArray              *linkLocations;
@property (nonatomic,strong)    BDM80AttributedLabelURL       *touchedLink;
...
@implementation BDM80AttributedLabel
...
- (void)setFont:(UIFont *)font
{
    if (font && _font != font)
    {
        ...
        [self resetFont];
        for (BDM80AttributedLabelAttachment *attachment in _attachments)
        {
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (BDM80AttributedLabelURL *url in _linkLocations)
        {
...
- (BDM80AttributedLabelURL *)urlForPoint:(CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            BDM80AttributedLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    BDM80AttributedLabelURL *url = [self urlForPoint:point];
...
- (BDM80AttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (BDM80AttributedLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment:(BDM80AttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
...
{
    BDM80AttributedLabelAttachment *attachment = [BDM80AttributedLabelAttachment attachmentWith:image
                                                                                     margin:margin
                                                                             alignment:alignment
                                                                               maxSize:maxSize];
...
- (void)appendView:(UIView *)view
         ...
         alignment:(M80ImageAlignment)alignment
{
    BDM80AttributedLabelAttachment *attachment = [BDM80AttributedLabelAttachment attachmentWith:view
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:CGSizeZero];
    [self appendAttachment:attachment];
...
{
    BDM80AttributedLabelURL *url = [BDM80AttributedLabelURL urlWithLinkData:linkData
                                                                  range:range
                                                                  color:color];
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            BDM80AttributedLabelAttachment* attributedImage = (BDM80AttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (void)computeLink:(NSString *)text
{
    ...
    [BDM80AttributedLabelURLDetector.shared detectLinks:text
                                           completion:^(NSArray<BDM80AttributedLabelURL *> * _Nullable links) {
                                               ...
                                               if ([text isEqualToString:plainText])
                                               {
                                                   ...
                                                   if ([links count])
                                                   {
                                                       for (BDM80AttributedLabelURL *link in links)
                                                       {
...
- (void)addAutoDetectedLink:(BDM80AttributedLabelURL *)link
{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    BDM80AttributedLabelURL *touchedLink = [self urlForPoint:point];
