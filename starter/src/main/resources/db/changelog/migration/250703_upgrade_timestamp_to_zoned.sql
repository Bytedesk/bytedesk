-- 250703_upgrade_timestamp_to_zoned.sql
-- MySQL版本：将 bytedesk_ 表中的时间戳字段从 DATETIME 升级到 TIMESTAMP
-- 此脚本将所有 bytedesk_ 表的 created_at 和 updated_at 列
-- 从 DATETIME (LocalDateTime) 转换为 TIMESTAMP (ZonedDateTime)
...
-- 创建游标来遍历所有需要更新的表
DELIMITER //
DROP PROCEDURE IF EXISTS update_timestamp_columns //
CREATE PROCEDURE update_timestamp_columns()
BEGIN
    ...
    read_loop: LOOP
        ...
        -- 输出信息
        SELECT CONCAT('Updated table: ', cur_table) AS message;
    END LOOP;
...
-- 添加注释
SELECT 'Migration completed. All bytedesk_ tables with created_at and updated_at columns have been updated from DATETIME to TIMESTAMP.' AS message;
SELECT 'created_at: Creation timestamp with timezone (upgraded from DATETIME to TIMESTAMP for internationalization)' AS note1;
SELECT 'updated_at: Last update timestamp with timezone (upgraded from DATETIME to TIMESTAMP for internationalization)' AS note2;
