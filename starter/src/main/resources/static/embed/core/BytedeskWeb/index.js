...
class J {
  ...
  async _initVisitor() {
    ...
    const t = localStorage.getItem(S), e = localStorage.getItem(E);
    ...
    return t && e && o ? (console.log("访客信息相同，直接返回本地访客信息"), (r = (a = this.config).onVisitorInfo) == null || r.call(a, t || "", e || ""), {
      ...
      async ({ initVisitor: m }) => {
        var h, l, p, d, f, w, I, M, y, T, C, D, k, $, c, v;
        try {
          const x = {
            ...
            extra: typeof ((w = this.config.chatConfig) == null ? void 0 : w.extra) == "string" ? this.config.chatConfig.extra : JSON.stringify(((I = this.config.chatConfig) == null ? void 0 : I.extra) || {})
          }, g = await m(x);
          return console.log("访客初始化API响应:", g.data, x), ((M = g.data) == null ? void 0 : M.code) === 200 ? ((T = (y = g.data) == null ? void 0 : y.data) != null && T.uid && (localStorage.setItem(S, g.data.data.uid), console.log("已保存uid到localStorage:", g.data.data.uid)), (D = (C = g.data) == null ? void 0 : C.data) != null && D.visitorUid && (localStorage.setItem(
            E,
            g.data.data.visitorUid
          ), console.log(
            "已保存visitorUid到localStorage:",
            g.data.data.visitorUid
          )), (k = g.data) != null && k.data && (console.log("触发onVisitorInfo回调"), (c = ($ = this.config).onVisitorInfo) == null || c.call(
            $,
            g.data.data.uid || "",
            g.data.data.visitorUid || ""
          )), g.data.data) : (console.error("访客初始化失败:", (v = g.data) == null ? void 0 : v.message), null);
        } catch (x) {
          return console.error("访客初始化出错:", x), null;
        } finally {
  ...
  async _browseVisitor() {
    var t, e, i, o;
    try {
      const n = window.location.href, s = document.title, a = document.referrer, r = navigator.userAgent, m = this.getBrowserInfo(r), h = this.getOSInfo(r), l = this.getDeviceInfo(r), p = `${screen.width}x${screen.height}`, d = new URLSearchParams(window.location.search), f = d.get("utm_source") || void 0, w = d.get("utm_medium") || void 0, I = d.get("utm_campaign") || void 0, M = localStorage.getItem(S), y = {
        url: n,
        title: s,
        referrer: a,
        userAgent: r,
        ...
        visitorUid: String(
          ((t = this.config.chatConfig) == null ? void 0 : t.uid) || M || ""
        ),
      ...
      const { browse: T } = await import("../../apis/visitor/index.js"), C = await T(y);
  ...
  async _getUnreadMessageCount() {
    return this.getUnreadMessageCountPromise ? (this.config.isDebug && console.log("获取未读消息数请求正在进行中，返回现有Promise"), this.getUnreadMessageCountPromise) : (this.getUnreadMessageCountPromise = import("../../apis/message/index.js").then(
      async ({ getUnreadMessageCount: t }) => {
        var e, i, o, n, s;
        try {
          const a = String(((e = this.config.chatConfig) == null ? void 0 : e.visitorUid) || ""), r = localStorage.getItem(S), m = localStorage.getItem(E), h = {
            uid: r || "",
            visitorUid: a || m || "",
  ...
  // 清除本地访客信息，强制重新初始化
  clearVisitorInfo() {
    localStorage.removeItem(S), localStorage.removeItem(E), this.config.isDebug && console.log("已清除本地访客信息");
  }
  // 强制重新初始化访客信息（忽略本地缓存）
  ...
  async clearUnreadMessages() {
    return this.clearUnreadMessagesPromise ? (this.config.isDebug && console.log("清空未读消息请求正在进行中，返回现有Promise"), this.clearUnreadMessagesPromise) : (this.clearUnreadMessagesPromise = import("../../apis/message/index.js").then(
      async ({ clearUnreadMessages: t }) => {
        var e, i;
        try {
          const o = String(((e = this.config.chatConfig) == null ? void 0 : e.visitorUid) || ""), n = localStorage.getItem(S), s = localStorage.getItem(E), a = {
            uid: n || "",
            visitorUid: o || s || "",
            orgUid: ((i = this.config.chatConfig) == null ? void 0 : i.org) || ""
          }, r = await t(a);
  ...
  createBubble() {
    ...
    if ((l = this.config.bubbleConfig) != null && l.show) {
      ...
      g.textContent = ((w = this.config.bubbleConfig) == null ? void 0 : w.title) || "", g.style.fontWeight = "bold", g.style.color = ((I = this.config.theme) == null ? void 0 : I.mode) === "dark" ? "#e5e7eb" : "#1f2937", g.style.marginBottom = "4px", g.style.textAlign = this.config.placement === "bottom-left" ? "left" : "right", x.appendChild(g);
      const b = document.createElement("div");
      b.textContent = ((M = this.config.bubbleConfig) == null ? void 0 : M.subtitle) || "", b.style.fontSize = "0.9em", b.style.color = ((y = this.config.theme) == null ? void 0 : y.mode) === "dark" ? "#9ca3af" : "#4b5563", b.style.textAlign = this.config.placement === "bottom-left" ? "left" : "right", x.appendChild(b), c.appendChild(x), e.appendChild(c);
      ...
      `, e.appendChild(_), e.appendChild(V), t.appendChild(e), setTimeout(() => {
        e && (e.style.opacity = "1", e.style.transform = "translateY(0)");
      }, 500);
    }
    this.bubble = document.createElement("button");
    const i = this.config.buttonConfig || {}, o = i.width || 60, n = i.height || 60, s = Math.min(o, n) / 2, a = ((D = this.config.theme) == null ? void 0 : D.mode) === "dark", r = a ? "#3B82F6" : "#0066FF", m = ((k = this.config.theme) == null ? void 0 : k.backgroundColor) || r;
    ...
    if (this.bubble.appendChild(h), this.bubble.addEventListener("mouseenter", () => {
      ...
      let c = 0, v = 0, x = 0, g = 0;
      this.bubble.addEventListener("mousedown", (b) => {
        b.button === 0 && (this.isDragging = !0, c = b.clientX, v = b.clientY, x = t.offsetLeft, g = t.offsetTop, t.style.transition = "none");
      }), document.addEventListener("mousemove", (b) => {
        if (!this.isDragging) return;
        b.preventDefault();
        const _ = b.clientX - c, V = b.clientY - v, B = x + _, P = g + V, L = window.innerHeight - t.offsetHeight;
  ...
  createChatWindow() {
    ...
    t ? this.window.style.cssText = `
      ...
      ` : this.window.style.cssText = `
        ...
        overflow: hidden;
        z-index: 10000;
        transition: all ${(l = this.config.animation) == null ? void 0 : l.duration}ms ${(p = this.config.animation) == null ? void 0 : p.type};
      `;
    ...
    `, s.src = this.generateChatUrl(), console.log("iframe.src: ", s.src), this.window.appendChild(s), this.config.showSupport) {
      const f = document.createElement("div"), w = ((d = this.config.theme) == null ? void 0 : d.mode) === "dark";
      f.style.cssText = `
        height: 24px;
        display: flex;
        align-items: center;
  ...
  generateChatUrl(t = "messages") {
    ...
    const e = new URLSearchParams(), i = localStorage.getItem(S), o = localStorage.getItem(E);
  ...
  handleLocalStorageData(t) {
    ...
    const o = localStorage.getItem(S), n = localStorage.getItem(E);
    if (o === e && n === i) {
      console.log("handleLocalStorageData: 值相同，跳过设置");
      return;
    }
    localStorage.setItem(S, e), localStorage.setItem(E, i), console.log("handleLocalStorageData: 已更新localStorage", {
      uid: e,
      visitorUid: i
