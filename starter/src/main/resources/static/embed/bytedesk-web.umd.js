(function(m,r){typeof exports=="object"&&typeof module<"u"?module.exports=r():typeof define=="function"&&define.amd?define(r):(m=typeof globalThis<"u"?globalThis:m||self,m.BytedeskWeb=r())})(this,function(){"use strict";var H=Object.defineProperty;var _=(m,r,t)=>r in m?H(m,r,{enumerable:!0,configurable:!0,writable:!0,value:t}):m[r]=t;var g=(m,r,t)=>_(m,typeof r!="symbol"?r+"":r,t);class m{constructor(t){g(this,"config");g(this,"bubble",null);g(this,"window",null);g(this,"inviteDialog",null);g(this,"contextMenu",null);g(this,"hideTimeout",null);g(this,"isVisible",!1);g(this,"isDragging",!1);g(this,"windowState","normal");g(this,"loopCount",0);g(this,"loopTimer",null);this.config={...this.getDefaultConfig(),...t}}getDefaultConfig(){return{isDebug:!1,isPreload:!1,baseUrl:"https://cdn.weiyuai.cn/chat",placement:"bottom-right",marginBottom:20,marginSide:20,autoPopup:!1,inviteConfig:{show:!1,text:"邀请您加入对话",acceptText:"开始对话",rejectText:"稍后再说"},tabsConfig:{home:!1,messages:!0,help:!1,news:!1},bubbleConfig:{show:!0,icon:"👋",title:"需要帮助吗？",subtitle:"点击开始对话"},buttonConfig:{show:!0,width:60,height:60,onClick:()=>{this.showChat()}},showSupport:!0,chatConfig:{org:"df_org_uid",t:"2",sid:"df_rt_uid"},animation:{enabled:!0,duration:300,type:"ease"},theme:{mode:"system",textColor:"#ffffff",backgroundColor:"#0066FF"},window:{width:380,height:640},draggable:!1,locale:"zh-cn"}}init(){var t;this.createBubble(),this.createInviteDialog(),this.setupMessageListener(),this.setupResizeListener(),this.preload(),this.config.autoPopup&&setTimeout(()=>{this.showChat()},this.config.autoPopupDelay||1e3),(t=this.config.inviteConfig)!=null&&t.show&&setTimeout(()=>{this.showInviteDialog()},this.config.inviteConfig.delay||3e3)}createBubble(){var p,w,l,f,y,k,$,I,D,S,L,z,B;const t=document.createElement("div");t.style.cssText=`
    ...
    `;let e=null;if((p=this.config.bubbleConfig)!=null&&p.show){e=document.createElement("div"),e.style.cssText=`
      ...
      `,s.setAttribute("data-placement",this.config.placement||"bottom-right");const C=document.createElement("span");C.textContent=((f=this.config.bubbleConfig)==null?void 0:f.icon)||"",C.style.fontSize="20px",s.appendChild(C);const v=document.createElement("div"),x=document.createElement("div");x.textContent=((y=this.config.bubbleConfig)==null?void 0:y.title)||"",x.style.fontWeight="bold",x.style.color=((k=this.config.theme)==null?void 0:k.mode)==="dark"?"#e5e7eb":"#1f2937",x.style.marginBottom="4px",x.style.textAlign=this.config.placement==="bottom-left"?"left":"right",v.appendChild(x);const c=document.createElement("div");c.textContent=(($=this.config.bubbleConfig)==null?void 0:$.subtitle)||"",c.style.fontSize="0.9em",c.style.color=((I=this.config.theme)==null?void 0:I.mode)==="dark"?"#9ca3af":"#4b5563",c.style.textAlign=this.config.placement==="bottom-left"?"left":"right",v.appendChild(c),s.appendChild(v),e.appendChild(s);const E=document.createElement("div");E.style.cssText=`
        position: absolute;
        bottom: -6px;
      ...
      `,e.appendChild(E),e.appendChild(T),t.appendChild(e),setTimeout(()=>{e&&(e.style.opacity="1",e.style.transform="translateY(0)")},500)}this.bubble=document.createElement("button");const i=this.config.buttonConfig||{},n=i.width||60,o=i.height||60,a=Math.min(n,o)/2,h=((L=this.config.theme)==null?void 0:L.mode)==="dark",b=h?"#3B82F6":"#0066FF",u=((z=this.config.theme)==null?void 0:z.backgroundColor)||b;this.bubble.style.cssText=`
      background-color: ${u};
      width: ${n}px;
      height: ${o}px;
      border-radius: ${a}px;
    ...
    `,i.icon){const s=document.createElement("span");s.textContent=i.icon,s.style.fontSize=`${o*.4}px`,d.appendChild(s)}else{const s=document.createElement("div");s.innerHTML=`
      ...
      `,d.appendChild(s)}if(this.bubble.appendChild(d),this.bubble.addEventListener("mouseenter",()=>{this.bubble.style.transform="scale(1.1)"}),this.bubble.addEventListener("mouseleave",()=>{this.bubble.style.transform="scale(1)"}),t.appendChild(this.bubble),this.config.draggable){let s=0,C=0,v=0,x=0;this.bubble.addEventListener("mousedown",c=>{c.button===0&&(this.isDragging=!0,s=c.clientX,C=c.clientY,v=t.offsetLeft,x=t.offsetTop,t.style.transition="none")}),document.addEventListener("mousemove",c=>{if(!this.isDragging)return;c.preventDefault();const E=c.clientX-s,T=c.clientY-C,M=v+E,W=x+T,R=window.innerHeight-t.offsetHeight;M<=window.innerWidth/2?(t.style.left=`${Math.max(0,M)}px`,t.style.right="auto",t.style.alignItems="flex-start",this.config.placement="bottom-left"):(t.style.right=`${Math.max(0,window.innerWidth-M-t.offsetWidth)}px`,t.style.left="auto",t.style.alignItems="flex-end",this.config.placement="bottom-right"),t.style.bottom=`${Math.min(Math.max(0,window.innerHeight-W-t.offsetHeight),R)}px`}),document.addEventListener("mouseup",()=>{this.isDragging&&(this.isDragging=!1,t.style.transition="all 0.3s ease",this.config.marginSide=parseInt(this.config.placement==="bottom-left"?t.style.left:t.style.right)||20,this.config.marginBottom=parseInt(t.style.bottom||"20"))})}this.bubble.addEventListener("click",()=>{if(!this.isDragging){console.log("bubble click");const s=this.bubble.messageElement;s instanceof HTMLElement&&(s.style.display="none"),this.showChat()}}),this.bubble.messageElement=e,document.body.appendChild(t),this.bubble.addEventListener("contextmenu",s=>{this.showContextMenu(s)}),document.addEventListener("click",()=>{this.hideContextMenu()})}getSupportText(){var i;const t=((i=this.config)==null?void 0:i.locale)||"zh-cn",e={"zh-cn":"微语技术支持","zh-tw":"微語技術支援",en:"Powered by Bytedesk",ja:"Bytedeskによる技術支援",ko:"Bytedesk 기술 지원"};return e[t]||e["zh-cn"]}createChatWindow(){var h,b,u,d,p,w,l;this.window=document.createElement("div");const t=window.innerWidth<=768,e=window.innerWidth,i=window.innerHeight,n=Math.min(((h=this.config.window)==null?void 0:h.width)||e*.9,e*.9),o=Math.min(((b=this.config.window)==null?void 0:b.height)||i*.9,i*.9);t?this.window.style.cssText=`
        ...
        overflow: hidden;
        transition: all ${(u=this.config.animation)==null?void 0:u.duration}ms ${(d=this.config.animation)==null?void 0:d.type};
    ...
    `,a.src=this.generateChatUrl(),console.log("iframe.src: ",a.src),this.window.appendChild(a),this.config.showSupport){const f=document.createElement("div"),y=((l=this.config.theme)==null?void 0:l.mode)==="dark";f.style.cssText=`
        height: 24px;
        display: flex;
        align-items: center;
        justify-content: center;
      ...
      max-width: 300px;
      text-align: center;
    `,(b=this.config.inviteConfig)!=null&&b.icon){const l=document.createElement("div");l.style.cssText=`
        font-size: 32px;
        margin-bottom: 12px;
        color: ${t?"#e5e7eb":"#333"};
      ...
      margin-bottom: 16px;
      color: ${t?"#e5e7eb":"#333"};
    `,e.textContent=((u=this.config.inviteConfig)==null?void 0:u.text)||"需要帮助吗？点击开始对话",this.inviteDialog.appendChild(e);const i=document.createElement("div");i.style.cssText=`
      display: flex;
      gap: 10px;
      justify-content: center;
    `;const n=document.createElement("button");n.textContent=((d=this.config.inviteConfig)==null?void 0:d.acceptText)||"开始对话";const o=((p=this.config.theme)==null?void 0:p.backgroundColor)||(t?"#3B82F6":"#0066FF");n.style.cssText=`
      padding: 8px 16px;
      background: ${o};
      color: white;
      border: none;
      border-radius: 4px;
      cursor: pointer;
    `,n.onclick=()=>{var l,f;this.hideInviteDialog(),this.showChat(),(f=(l=this.config.inviteConfig)==null?void 0:l.onAccept)==null||f.call(l)};const a=document.createElement("button");a.textContent=((w=this.config.inviteConfig)==null?void 0:w.rejectText)||"稍后再说",a.style.cssText=`
      padding: 8px 16px;
