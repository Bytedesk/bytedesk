import e from "../../utils/index.js";
import t from "../speedometer/index.js";
import n from "../throttle/index.js";
//#region node_modules/.pnpm/axios@1.19.0/node_modules/axios/lib/helpers/progressEventReducer.js
var r = (e, r, i = 3) => {
	let a = 0, o = t(50, 250);
	return n((t) => {
		if (!t || typeof t.loaded != "number") return;
		let n = t.loaded, i = t.lengthComputable ? t.total : void 0, s = Math.max(0, i == null ? n : Math.min(n, i)), c = Math.max(0, s - a), l = o(c);
		a = Math.max(a, s), e({
			loaded: s,
			total: i,
			progress: i ? s / i : void 0,
			bytes: c,
			rate: l || void 0,
			estimated: l && i ? (i - s) / l : void 0,
			event: t,
			lengthComputable: i != null,
			[r ? "download" : "upload"]: !0
		});
	}, i);
}, i = (e, t) => {
	let n = e != null;
	return [(r) => t[0]({
		lengthComputable: n,
		total: e,
		loaded: r
	}), t[1]];
}, a = (t, n = e.asap) => (...e) => n(() => t(...e));
//#endregion
export { a as asyncDecorator, i as progressEventDecorator, r as progressEventReducer };
