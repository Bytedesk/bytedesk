 ...
 * 策略（刻意保持保守，避免影响实时通讯）：
 *   1. 导航请求 (mode === 'navigate')：网络优先，失败时回退缓存的页面，再回退 offline.html
 *   2. 同源静态资源 (/assets/ 下的 js/css/img/font 及 /icons/)：stale-while-revalidate（文件名带 hash，可安全缓存）
 *   3. 其余请求（API、跨域等）直接走网络，不做任何缓存
 */
const SW_VERSION = "bytedesk-web-v2";
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    (async () => {
      ...
      // 预缓存离线兜底页；入口页运行时缓存，避免预缓存与 hash 资源不一致
